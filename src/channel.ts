...
export function channelRemoveOwnerV2(token: string, channelId: number, uId: number) {
  ...
  const authUserId: number = getUIdFromToken(token) as number;
  if (!isMember(channelId, authUserId) || (!isOwner(channelId, authUserId) && !isGlobalOwnerFromToken1(token))) {
...
export const channelAddownerV2 = (token: string, channelId: number, uId: number) => {
  ...
  if ((!isOwnerByToken(channelId, token) && !isGlobalOwnerFromToken1(token)) || !isMember(channelId, getUIdFromToken(token))) { // has problem here
